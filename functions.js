...
function getUp(obj){
  ...
  while(up.nodeType==8||(up.nodeType==3&&up.nodeValue.replace(/^\s*|\s*$/g,"")=="")){//条件满足时，接着再找(条件为注释节点或者文本节点中为空字符串时，条件满足)
...
function getNext(obj){
  var next=obj.nextSibling;
   if(next==null){
    ...
    while(next.nodeType==8||(next.nodeType==3&&next.nodeValue.replace(/^\s*|\s*$/g,"")=="")){//条件满足时，接着再找(条件为注释节点或者文本节点中为空字符串时，条件满足)
